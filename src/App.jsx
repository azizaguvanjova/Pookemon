...
export default function App() {
  ...
  /* Challenge
	...
	Pookachu'nun hareket etmesi ve hapishaneden kaçması için aşağıdaki updatePosition fonksiyonunun tamamlanması gerekiyor. Göreviniz fonksiyonu aşağıdaki gibi tamamlamaktır:
		...
		1. updatePosition fonksiyonu çağrıldığında, pookachu state nesnesini (satır 49) boundaries state nesnesine (satır 56) uygun olarak bir sonraki adımda açıklanan 
    şekilde güncellemek için setPookachu fonksiyonunu kullanmalıdır. Bu iki state ile, aşağıdaki özellikler bu challenge için özellikle önemlidir: 
		...
		2. pookachu state nesnesi aşağıdaki gibi güncellenmelidir: 
				...
				- Kullanıcı Pookachu'nun oyunun sınırlarından birini geçmesini sağlamaya çalışıyorsa (ilgili eksendeki maksimum değerin üzerinde veya minimum değerin altında 
        - yöneldiği yönün ekseni), o zaman mevcut konumu değişmemeli ve pookachu state nesnesinin tüm mevcut özellik değerleri korunmalıdır. 
				   
				- Poocahu henüz kullanıcının gitmesini istediği yönün sınırına ulaşmamışsa, xPosition veya yPosition (" target özelliği ") aşağıdaki tabloya uygun olarak
         güncellenmelidir. pookachu state nesnesinin diğer tüm özellik değerleri korunmalıdır.
	...
	+--------------------+-------------------------------------------------+----------------------------+
		...
		3. Görevi tamamladığınızda, hapishanenin sınırları içinde kalırken ekrandaki yön tuşlarını veya klavyenizdeki ok tuşlarını kullanarak Pookachu'yu herhangi bir yöne 
    hareket ettirebilmelisiniz. Ayrıca A ve B tuşlarını kullanarak kapıyı patlatırsanız hapishaneden kaçabilmeniz gerekiyor
*/

  function updatePosition() {
   setPookachu((prevPookachu) => {
    let newPosition = {...prevPookachu}
